...
$seo['upline'] = $seo['title'] = 'Lexique | Maisons Kerbea ';
...
$select_lexique = $PDO -> query('SELECT * FROM lexique ORDER BY titre_lexique ASC');
$select_lexique = $select_lexique -> fetchAll();
$array_letters = array();

if (count($select_lexique)) {
    foreach($select_lexique as $lexique){
        if (!in_array($lexique['titre_lexique'][0], $array_letters)) {
            $array_letters[] = $lexique['titre_lexique'][0];
        }
    }
}
?>
        ...
        <script>
            $(function (){
                ...
                function switchLetter( letter ){
                    $('.title-lexique, .text-lexique').hide();
                    $('.l-' + letter).show();
                }
                ...
                $('.letters span').click(function (){
                    ...
                    switchLetter($(this).text().toLowerCase());
                });

                $('.letters span:first').click();
            });
        </script>
	</head>
	<body>
        ...
        <div id="container-content">
            ...
            <div class="content-hover">
                <div class="site-width relative">
                    ...
                    <div class="breadcrumbs"><a href="<?php echo RESSOURCE_URL; ?>">Maisons Kerbéa</a><img src="<?php echo RESSOURCE_URL; ?>/images/pictos/fl-r-red.png" alt=""/>Lexique</div>

                    <h1 class="black-title">Lexique</h1>
                    <?php
                        if(count($select_lexique)): ?>
                            <div class="letters">
                            <?php
                            ...
                            <?php
                            foreach($select_lexique as $lexique):
                                $f_letter = strtolower($lexique['titre_lexique'][0]);
                                ?>
                                <h3 class="title-lexique l-<?php echo $f_letter; ?>"><?php echo $lexique['titre_lexique']; ?></h3>
                                <div class="text-lexique l-<?php echo $f_letter; ?>"><?php echo nl2br($lexique['text_lexique']); ?></div>
                                <?php
                            endforeach;
                            ?>
                            </div>
                        <?php else: ?>
                        <p class="pdg_m">Il n'y a aucun lexique pour le moment.</p>
                    <?php endif; ?>
                </div>
            </div>
        </div>
