...
$seo['upline'] = $seo['title'] = 'Actualités | Maisons Kerbea';
...
if($_SESSION['agence']['session']){
    ...
    $select_actualite = $PDO -> query('SELECT * FROM actualite WHERE publier_actu = 1 ORDER BY idOrdre_actu DESC;');
}
...
<div id="container-content">
    ...
    <div class="content-hover">
        <div class="site-width">
            <div class="breadcrumbs"><a href="<?php echo RESSOURCE_URL;if(!empty($_SESSION['agence']['alias'])){ echo '/agence-'.$_SESSION['agence']['alias'];} ?>">Maisons Kerbéa<?php if(!empty($_SESSION['agence']['alias'])){ echo ' '.$_SESSION['agence']['alias'];} ?></a><img src="<?php echo RESSOURCE_URL; ?>/images/pictos/fl-r-red.png" alt=""/>Listes des actualités</div>
            <h1 class="black-title">Actualités</h1>
            <div class="fiche">
                <div id="container-actu" class="list-actu">
                    <?php
                    if($select_actualite -> rowCount()):
                        foreach( $select_actualite as $actualite):
                            $link= !empty($_SESSION['agence']['alias']) ? RESSOURCE_URL.'/agence-'.$_SESSION['agence']['alias'].'/actualites/'.rewrite_nom($actualite['titre_actu'].'-'.$actualite['id_actu']) : RESSOURCE_URL.'/actualites/'.rewrite_nom($actualite['titre_actu'].'-'.$actualite['id_actu']) ;
                            ...
                            <div class="actu">
                                ...
                                <div class="title"><?php echo $actualite['titre_actu']; ?></div>
                                <div class="text"><?php echo $actualite['descriptionCourte_actu']; ?></div>
                                <?php echo createLink('lire la suite',$link,'link'); ?>
                            </div>
                        <?php endforeach; ?>
                    <?php else : ?>
