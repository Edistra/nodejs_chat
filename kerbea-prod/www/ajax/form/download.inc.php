...
?>
		...
		$(function(){
			$('#contact_form').placeholder().submit(function(){
				if($('#contact_form').valid()){
					...
					$.post('<?php echo $base_url; ?>/ajax/postDownload.ajax.php',datas, function(data){
						if(data.error == 0){
							$('#contact_form').hide();
	...
	<form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post" id="contact_form">
		...
		<div class="form_item submit"><input type="submit" value="Télécharger la brochure (pdf)" /></div>
		<div class="both"></div>
	</form>
<?php
	else:
