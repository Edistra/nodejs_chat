<?php
	...
	$select_agence_sel = $PDO -> prepare('SELECT * FROM agence WHERE id_agence = :id_agence');
	$select_agence_sel -> execute(array('id_agence' => $_GET['id']));
...
?>
		...
		$(function(){
			$('#contact_form').placeholder().submit(function(){
				if($('#contact_form').valid()){
					...
					$.post('<?php echo $base_url; ?>/ajax/postContact_offre.ajax.php',datas, function(data){
						if(data.error == 0){
							$('#contact_form').hide();
	...
	<form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post" id="contact_form">
		...
		<input type="hidden" name="offre" id="offre" value=""/>
