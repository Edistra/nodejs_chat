<?php
	...
	* 200 : agence introuvable
	*/
	
	$array_retour = array();
	
	try{
		if(empty($_POST['prenom']) || empty($_POST['nom']) || empty($_POST['telephone']) || empty($_POST['email']) || empty($_POST['agence'])  || empty($_POST['modele'])){
		...
		$select_modele_maison -> execute(array('id_modele_maison' => $_POST['modele']));
		...
		$line_modele_maison = $select_modele_maison -> fetch();
		...
		$insert_demande_contact -> execute(array(
			...
			'id_modele_maison' => $line_modele_maison['id_modele_maison'],
		...
		$message_mail = '
			<table width="600" border="0" cellpadding="0" cellspacing="0">
				<tr>
					<td valign="top">
						<h1 style="font-family:Arial;font-size:16px;color:#333;">[Fiche modèle] - Demande de contact sur le site Maisons Kerbea</h1>
						<h2 style="font-family:Arial;font-size:12px;color:#696a6a;">Agence sélectionnée : '.$line_agence['nom_agence'].'.</h2>
...
//		$email_dest = EMAIL_ADMIN;
		...
		$mail_obj->Subject='[Contact] - Maisons Kerbea - contact d\'un internaute.';
